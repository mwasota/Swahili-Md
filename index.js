...
async function startgss() {
    ...
    const gss = gssConnect({
        ...
        browser: ['swahili md Multi Device','Safari','1.0.0'],
        auth: state,
        getMessage: async (key) => {
            ...
            return {
                conversation: "Hai Im swahili md"
            }
        }
    })
...
gss.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    const joinTime = moment.tz('Africa/Dodoma').format('HH:mm:ss');
                    const joinDate = moment.tz('Africa/Dodoma').format('DD/MM/YYYY');
                ...
                // Left message
                    ...
                    const userName = num.split('@')[0];
                    const leaveTime = moment.tz('Africa/Dodoma').format('HH:mm:ss');
                    const leaveDate = moment.tz('Africa/Dodoma').format('DD/MM/YYYY');
                    const membersCount = metadata.participants.length;
    ...
    if (connection === 'close') {
        ...
        gss.sendMessage(gss.user.id, {
            text: `*hi bro! 🫡*\n_swahili-md bot has successfully connected to the server_`
        });
    }
});
